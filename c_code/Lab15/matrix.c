...
void delete_matrix(matrix* A) {
    if (A != NULL) {
        // Free the allocated memory for the matrix values
        free(A->val);
        A->val = NULL; // Avoid dangling pointer
    }
}
...
void delete_vector(vector* v) {
    if (v != NULL) {
        // Free the allocated memory for the vector values
        free(v->val);
        v->val = NULL; // Avoid dangling pointer
    }
}